...
const SPECS_DIR = resolve(
  specsFlag >= 0 ? args.splice(specsFlag, 2)[1] : 'specs'
)
...
function validate(nodes) {
  ...
  for (const node of nodes.values()) {
    for (const dep of node.deps) {
      if (!nodes.has(dep))
        errors.push(`missing dep: ${node.id} -> ${dep} (in ${node.file})`)
    }
  }
...
if (cmd === 'graph') {
  ...
  for (const { id, depth } of result)
    console.log(`${'  '.repeat(depth)}- ${id}`)
} else {
  console.error(
    `usage: ceg.mjs <graph|validate|topo|impact <id>> [--specs <dir>]`
  )
  process.exit(2)
}
